...
绝对耗时比较。**表里的「按钮 3,217」也已作废**——那是虚拟化之前、并且是长廊剧场
重构之前的计数，见本文末「2026-07-31 结构计数重新基线」。
...
（这一行的「按钮 43」同样不再是当前基线：长廊剧场重构改了一条消息渲染多少控件。
同样张、同视口、同为 6 条已挂载消息的 2026-07-31 复测是 46，见文末一节。）
...
- 两次切换的 DOM/heap 采样结束后，历史用户消息执行「编辑中跳顶 → 返回 → 保存 →
  再跳顶/返回」完整链路，并从 ST 状态、普通虚拟行重挂载后的 DOM 和草稿 store 三处
  回读；角色消息另从真实 `⋯ → Edit` 入口保存；
- 没有页面异常、ChatUI 控制台错误或未关闭弹窗；
- 截图实际为 1920×1080。

编辑验收刻意放在计时和结构采样之后：保存会把一条很长的富文本消息替换成短测试
marker；若先编辑再采样，会人为减少 formatter、iframe/DOM 和内存工作量，使切换结果
失真。报告 schema v3 把验收证据单列在 `editAcceptance`，不混入 `transitions`。
...
同一脚本的编辑钉行断言也会跑在 10 楼对照上。它先记录未编辑时的真实顶部窗口，再
要求编辑期间除目标钉行外的索引逐项不变；因此短样张可以合法地出现「顶部 7 行 +
第 16 条钉行 = 8 行」，同时仍会拒绝把 0–16 中间整段一起挂载的假窗口化。这里不再
依赖「编辑目标至少离视口 50 条」这种只对 400 楼成立的测试假设。
...
## 2026-07-31 结构计数重新基线（长廊剧场重构之后）

### 为什么旧的按钮数不能再用

长廊剧场重构改了「一条消息渲染多少个控件」这件事本身，所以本文早前记的按钮数不再
是同一个量的计数，不能拿来判断回归：

- **pr5(`937e25e`)** 重排了操作条。此前是两套词汇：用户消息平铺全部动作、角色消息
  把同样的动作全藏在 ⋯ 后面。现在按「对这一楼做什么 / 拿这一楼做什么」分：平铺
  编辑 / 删除 / ⋯ 三枚，只有 ST 真会重新生成的那一条（末条角色回复）多一枚重写；
  复制、复制原文、从此楼开分支、在此楼设检查点、隐藏此楼进 ⋯。
- **pr6(`7b136fb`/`f5c1d47`)** 把 swipe 的 `‹n/m›` 计数换成逐候选的刻度段，**每段
  是一个可点控件**。于是单行控件数第一次变成随数据增长的量——虽然刻度带窗口化，
  超出窗口只画固定条数并退回等宽计数。

因此 2026-07-15 基线那行「按钮 3,217」（虚拟化之前，800 条 article 全挂）与
2026-07-16 兼容性采样那行「按钮 43」都已作废。

### 新基线（出处：本轮实测）

命令与环境：`SILLYTAVERN_TEST_ROOT=~/Developer/SillyTavern node
scripts/e2e/measure-long-chat.mjs`（默认参数：0 轮预热、每种状态 1 个正式样本），
栈头 `271f795`，pinned SillyTavern 1.18.0（`51ad27f`），Chromium 1920×1080，
`long-plain`（400 楼 / 800 条），原生截断守卫按产品默认开启。报告落在
`test-results/performance/long-plain.json`。

| 状态 | 就绪时间 | long task 总时长 | DOM 元素 | 原生消息 | 已挂载 article | 按钮 | CDP 堆 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `disabled` | 4465 ms | 549 ms | 15,439 | 100 | 0 | 0 | 20.9 MiB |
| `bootstrap` | 3432 ms | 478 ms | 15,452 | 100 | 0 | 0 | 27.1 MiB |
| `active` | 3439 ms | 449 ms | 9,429 | 1 | 12 | 57 | 28.8 MiB |

单样本，且这一轮 `disabled` 比 `active` 还慢一秒——正是本文一贯说的墙钟噪声，
**这节的产出是结构计数，耗时只作诊断**。

楼层轨的功能门同轮全过：滚轮把刻度窗从第 324 楼移到第 319 楼，同时消息列表的
`scrollTop` 变化为 `0`（这是「楼层轨滚轮不误伤主滚动」唯一的自动化断言）；
Home/End 跳转、首楼预览文案与 `第 400 楼` 编号全部命中；采样期间没有超过 50 ms 的
帧间隔，单次最大 17.5 ms。

### 按钮落在哪里（同样张、同视口的独立复核跑）

上表只给总数，看不出「哪些随对话长度增长」。用一次性 harness 在同一份样张、同一
视口下复核了一遍（结构数逐项与门禁一致：57 / 12 / 9,429）：

| 区域 | `long-plain` | `long-rich` |
| --- | ---: | ---: |
| 书脊 spine | 3 | 3 |
| 场刊 playbill | 4 | 4 |
| topbar | 7 | 7 |
...
- `long-plain` 是 `11 × 3 + 1 × 5`——每一楼平铺 编辑 / 删除 / ⋯ 三枚，末条角色回复
  另加 重写 与 swipe 组的「下一版本」。
- `long-rich` 是 `[3, 3, 3, 4, 3, 11]`。那个 `4` 是正文里带代码块的行——代码块自带
  一枚复制钮；那个 `11` 是末楼：它有 **53 个候选回复**，而实际画出的刻度段是 **5**
  段（`swipeSegments = 5`），加上左右两枚 chevron 与四枚动作钮。**pr6 的窗口化在真
  机上确实生效**：53 个候选没有变成 53 个控件。

于是当前的量级规律是

```text
按钮 ≈ chrome 19 + 3 × 已挂载消息 + 末楼附加(重写 + 2 枚 chevron + ≤5 段刻度)
       + 每个可见代码块 1
```

——随**已挂载**消息数增长，不随完整对话长度增长（两份样张都是 800 条历史）。那个
`19` 只对「场刊里一张卡」成立：卡片的操作坞是每卡两枚，所以 chrome 这一项随当前
角色的对话条数走，不随消息数走。系统行不是「一楼」，不平铺任何动作，只有 ⋯。

同轮 `measure-chat-switch`（`long-rich-switch` 双 400 楼，走真实侧栏 A→B→A）给出
第三个点：`content ready` 时 6 条 article / 48 枚按钮，`settled` 后 8 条 / 54 枚，
两个方向一致。

### 一句必须写明的限度

「43 → 46」不是一次可归因的差分。两次采样之间变的不只是操作条与刻度：侧栏形状换成
了书脊＋场刊、原生截断守卫的默认值在 2026-07-19 翻开（所以 DOM 元素从 25,747 掉到
10,456）。所以那两个数只能当作**量级没变**的旁证——重构没有把单行控件数抬上一个数
量级——不能拿来给 pr5/pr6 各自记账。要做真正的差分归因，得在同一 runtime 上只翻一
个变量重测。
