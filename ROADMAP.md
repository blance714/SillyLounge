...
| **④ 输入框** | 🟢 ~88% | ＋菜单(置顶磁贴 + 工具列表 + wand 动态)、textarea、选择框 B、发送/停止、附件 chips;**M-C**:QR 悬浮条(镜像 #qr--bar,含 popout)、单/多行切换 | ＋菜单**拖拽/置顶/开关编辑器**(依赖 §7 独立配置面)、批量删除磁贴(待 ChatUI 自有多选 UI) |
...
| **配置系统**(§7 十项) | 🟡 ~38% | **薄地基**(M-E)+ 标头群/单两套(M-D)+ 单/多行(M-C):`config-store`(createStore 工厂)+ `adapter/config.js` 往返 + 设置面四个 select 双向同步;侧栏三形态/群单标头/输入框行数均持久化;`plusPinned` 配置驱动磁贴(无编辑器) | §7 剩余项、**独立(非内联)设置面**、＋菜单拖拽/置顶编辑器、选择框槽位可配 |

**毛估**:北极星完整度 ≈68-72%;"能当日常聊天用"体感 ≈90%(已部分 live-test:删除/swipe/滚动/布局)。
...
### 长尾
配置贴边浮层(§2.2 (b)/modal)+ 手机全屏 tab+返回(M-A 最大 UX 缺口)、群聊列表、搜索 🔍、Mode B、＋菜单拖拽/置顶编辑器、★ 收藏开关(需新 adapter 导出)、swipe 手势。
...
`cleanup/remove-legacy-phase12`(main 仍在基线 `c97d8c2`)。已提交至 `25b5620`:
`8b8e203`(写路径加固 + adapter 拆分 + store 工厂)→ `abf212f`(M-E 配置薄地基)→ `50cb5e6`(M-D 内容区)→ `6f2fa01`(M-B 顶栏)→ `f1c4d6b`/`e9e8185`/`4d8bb48`(M-C 输入框)→ `25b5620`(M-B/M-C 审查修复)。
**五大区主干已基本闭环**。下一阶段重心:**独立(非内联)配置面**(解锁 §7 + ＋菜单/选择框编辑器)、手机全屏 tab + 返回(M-A 最大 UX 缺口)、剩余模拟点击写路径迁移。
