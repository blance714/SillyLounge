...
- ~~**本机跑浏览器门禁时，宿主 checkout 里已装的同名扩展会顶掉被测产物。**~~ ——
  **2026-08-05 已根治**（先加了拒跑闸，随后换成不撞名）。ST 把 `express.static(public/)` 挂在 per-user 扩展路由
  ...
  发现方式是一条怎么都过不去的卡片断言，而磁盘上的树明明是对的。
  **最终做法**：固件改装到 `SillyLounge-e2e`,一个不会有人拿去安装的名字,重名从根上消失,
  维护者的安装原地不动、也不用每次挪来挪去。代价是固件不再复现真实安装的目录名——
  但 `src/` 里没有任何代码读自己的目录名(查过),所以这里没丢覆盖;目录名对真实安装的
  意义(改名会孤儿化所有已安装用户)记在 README 的两仓一节,那才是它约束的决策所在。
  两道保险都留着并实测过:重名时生成阶段直接拒跑;`smoke.spec.mjs` 经
  `getExtensionManifest()` 比对 manifest 里的树摘要,把 `EXTENSION_FOLDER` 改回撞名的
  旧值就会立刻变红。另外顺手修掉一个换名才暴露的连带问题:原先「关掉宿主上其它扩展」
  的名单硬编码放行 `third-party/sillylounge-dist`(那时正是固件自己的名字),换名后它会
  让维护者那份**继续被加载**,变成两个 SillyLounge 同台。
